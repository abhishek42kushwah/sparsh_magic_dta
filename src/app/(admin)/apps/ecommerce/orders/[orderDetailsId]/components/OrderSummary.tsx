...
const OrderSummary = ({ dataAmount }: { dataAmount?: OrderData }) => {
  // Safe fallback if dataAmount is undefined
  const orderItems = dataAmount?.orderItems || [];

  // Calculate subtotal
  const subtotal = orderItems.reduce((sum, item) => sum + (item.total || 0), 0);

  // Fallback values
  const discount = dataAmount?.discount ?? 0;
  const tax = +(subtotal * 0.18).toFixed(2);
  const shippingCost = dataAmount?.shippingCharge ?? 0;
  const covCharge = dataAmount?.codCharge ?? 0;

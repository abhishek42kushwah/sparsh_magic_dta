...
function AddWalletTxn() {
  const navigate = useNavigate();

  return (
    <ComponentContainerCard title="Add Wallet">
      <Formik
        initialValues={{
          userName: '',
          userId: '',
        ...
        onSubmit={async (values, { setSubmitting, resetForm }) => {
          try {
            const response = await fetch(`${BASE_URL}wallet-Txn`, {
              method: 'POST',
              headers: {
            ...
            if (data.success) {
              successToast('Wallet transaction added successfully');
              resetForm();
              navigate('/app/ecommerce/wallettxn');
            } else {
              errorToast(data.message || 'Failed to add wallet transaction');
            }
          } catch (error: any) {

...
const NewVisitors = () => {
  ...
  const chartOptions: ApexOptions = {
    ...
    ] as Array<{ name: string; data: number[] } & Record<string, any>>,
    chart: {
      height: 230,
      type: 'bar',
      toolbar: {
  ...
  return (
    <Card>
      <CardBody>
        <Row className="align-items-center">
          ...
          <Col xs={'auto'} className="align-self-center">
            <div className="img-group d-flex">
              {users.map((user, idx) => (
                <Link className="user-avatar position-relative d-inline-block ms-n2" to="" key={idx}>
        ...
        {Array.isArray(chartOptions.series) && typeof chartOptions.series[0] === 'object' && 'data' in chartOptions.series[0] && chartOptions.series[0].data?.length > 0 ? (

...
import {
  ...
  couponData,
  customers,
  ordersData, products, settingsData,
  CrmCallerList,
   LeadData,
  walletData, RxorderCrm,
  masterCategorys,
...
import type {
  ...
  WalletType,
  CrmCallerListType,
  RxOrderCrmType, RxOrderCrmListType,
  masterCategoryType,
  // OrderListType
} from '@/types/data'
...
export const getAllRxOrderCrmItems = async (): Promise<RxOrderCrmListType[]> => {
  const data = ordersData.map((order) => {
    const product = products.find((product) => product.id === order.productId)
    return {
      ...order,
...
export const getAllMasterCategory = async (): Promise<masterCategoryType[]> => {
  await sleep()
  return masterCategorys
}

// export const getAllOrderList = async (): Promise<OrderListType[]> => {
//   await sleep()
//   return orderList
// }

export const getAllRxOrderCrm = async (): Promise<RxOrderCrmType[]> => {
  await sleep()
  return RxorderCrm
}
...
export const getRxOrderCrmById = async (id: RxOrderCrmType['id']): Promise<RxOrderCrmType | undefined> => {
  const data = RxorderCrm.find((product) => product.id == id)
  await sleep()
  return data
}
...
export const getAllHomeBanner = async (): Promise<HomeBannerType[]> => {
  await sleep()
  return bannerData
}
export const getAllSettings = async (): Promise<SettingsType[]> => {
  await sleep()
  return settingsData
}
export const getAllCoupon = async (): Promise<CouponType[]> => {
  await sleep()
  return couponData
}

export const getAllLead = async (): Promise<LeadType[]> => {
  await sleep()
  return LeadData
}
export const getAllCRMCallerList = async (): Promise<CrmCallerListType[]> => {
  await sleep()
  return CrmCallerList
}

export const getAllWalletTxn = async (): Promise<WalletType[]> => {
  await sleep()
  return walletData
}
